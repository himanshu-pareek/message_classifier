...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 14 02:11:39 2018
...
for i in range (epochs):
    j = 0
    a = None
    train_error = np.zeros((n3, m))
    test_error = np.zeros ((n3, m))
    while j < X_train.shape[0]:
        ...
        Z2 = np.matmul (W2, sigmoid (Z1)) + b2

        Z3 = np.matmul (W3, sigmoid (Z2)) + b3

        a = softmax (Z3)

        da = calculate_da_squared (y_hat, a)
        ...
        dZ3 = sigmoid_grad (Z3) * da
        dW3 = np.matmul (dZ3, np.transpose (sigmoid (Z2)))
        db3 = (1 / m) * np.sum (dZ3, axis = 1, keepdims = True)

        dZ2 = np.multiply (sigmoid_grad (Z2), np.matmul (np.transpose (W3), dZ3))
        dW2 = np.matmul (dZ2, np.transpose (sigmoid (Z1)))
        db2 = (1 / m) * np.sum (dZ2, axis = 1, keepdims = True)

        dZ1 = np.multiply (sigmoid_grad (Z1), np.matmul (np.transpose (W2), dZ2))
    ...
    while j < X_train.shape[0]:
        ...
        Z3 = np.matmul (W3, sigmoid (Z2)) + b3
        a_ = sigmoid (Z3)
        train_error += calculate_loss_squared(y_hat, a_)
        j += m
    ...
    while j < X_test.shape[0]:
        ...
        Z3 = np.matmul (W3, sigmoid (Z2)) + b3
        a_ = sigmoid (Z3)
        test_error += calculate_loss_squared(y_hat, a_)
        j += m
